...
namespace Affin3D
{
    ...
    //        List<Point3D> normals_to_polys = new List<Point3D>();
    //        int startInd = 1;
    //        using (StreamReader sr = new StreamReader(fname))
    //        {
    //            string line = sr.ReadLine();
    ...
    //                        {
    //                            constructing_polyhedr = false;
    //                            res.Add(new Polyhedron(cur_points, cur_polygons, normals));
    //                            cur_points = new List<Point3D>();
    //                            cur_polygons = new List<List<int>>();
    //                            normals = new List<Point3D>();
    //                        }
    //                        if (NextChar == 'n')
    //                            normals.Add(ParcePoint(line.Replace('n', ' ')));
    //                        else
    //                            cur_points.Add(ParcePoint(line));
    //                    }
    ...
    //                {
    //                    constructing_polyhedr = true;
    //                    cur_polygons.Add(ParcePolygon(line, startInd));
    //                }
    //                else if (id == 's')
    //                {
    //                    var tokens = line.Substring(1).Trim();
    //                    int.TryParse(tokens, out startInd);
    //                }
    //                line = sr.ReadLine();
    //            }
    //        }
    ...
    //        return res;
    //    }
    //    private List<int> ParcePolygon(string line, int startInd)
    //    {
    //        List<int> res = new List<int>();
    //        var tokens = line.Substring(1).Trim().Split(' ').SkipWhile((x) => x == "").ToArray();
    //        for (var i = 0; i < tokens.Length; i++)
